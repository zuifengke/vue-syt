...
// 获取form组件实例
// 对于单个的表单，我们通过ref=属性名的方式就能够轻松获取到表单实例对象，从而使用该对象上的校验表单项的方法;
// 自定义变量 formRef 赋值给 ref 属性，通过formRef.value 拿到form表单的实例对象，通过调用validate方法对表单项目进行校验；
...
//点击用户登录按钮回调
const login = async () => {
  // 通过formRef.value 拿到form表单的实例对象，通过调用validate方法对表单项目进行校验；
